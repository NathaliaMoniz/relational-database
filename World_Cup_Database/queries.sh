...
echo -e "$($PSQL "SELECT name FROM teams WHERE team_id IN (
    SELECT winner_id FROM games WHERE round = 'Eighth-Final' AND year = 2014
    UNION
    SELECT opponent_id FROM games WHERE round = 'Eighth-Final' AND year = 2014) ORDER BY name ASC")"

echo -e "\nList of unique winning team names in the whole data set:"
echo -e "$($PSQL "SELECT name FROM teams WHERE team_id IN (SELECT winner_id FROM games) ORDER BY name ASC")"

echo -e "\nYear and team name of all the champions:"
echo -e "$($PSQL "SELECT games.year, teams.name FROM games JOIN teams ON teams.team_id = games.winner_id WHERE games.round = 'Final'")"
